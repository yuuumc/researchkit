 ...
 * - 关键术语用 [[term]] 包裹形成双链
 * - 自动生成 frontmatter (YAML) 带 tags 和 source
 * - 核心观点中出现的术语也会自动双链
 */
export function exportToObsidian(
  ...
  card.key_terms.forEach(item => {
    ...
    if (normalized.length >= 2) {
      termMap.set(normalized.toLowerCase(), normalized)
    }
  })

  // 把文本中出现的术语替换为 [[term]]
  const linkifyTerms = (text: string): string => {
    let result = text
  ...
  // 去重: 同一行多次出现只链第一次
  const linkifyOncePerLine = (text: string): string => {
    return text.split('\n').map(line => {
  ...
  // 生成 YAML frontmatter
  const tags = ['researchkit', 'knowledge-card']
  if (card.key_terms.length > 0) {
    tags.push('terms')
  }
  if (source) {
    tags.push('imported')
  ...
  // 方法论
  md += `## 🔧 方法论\n\n`
  md += `${linkifyOncePerLine(card.methodology)}\n\n`

  // 可操作建议
  md += `## ✅ 可操作建议\n\n`
  card.actionable_takeaways.forEach((item, i) => {
    md += `${i + 1}. ${linkifyOncePerLine(item)}\n`
  })
  md += `\n`
  ...
  // Obsidian 关系图提示
  md += `---\n`
  md += `*Generated by ResearchKit — 一站式 AI Research Agent*\n`
  md += `*双链格式：术语已用 \`[[term]]\` 包裹，导入 Obsidian 后可形成知识图谱。*\n`
