  ...
  return {
    ...
    outputSchema: {
      input: {
        ...
        bodyType: 'json',
        body: {
          goal: { type: 'string', required: false, minLength: 5, maxLength: 2000, description: 'Research goal (goal-based research mode)' },
          content: { type: 'string', required: false, minLength: 200, description: 'Full paper text (paper analysis mode, min 200 chars)' },
          title: { type: 'string', required: false, description: 'Paper title (paper analysis mode)' },
          source: { type: 'string', required: false, description: 'Source URL or filename (paper analysis mode)' },
          session_id: { type: 'string', required: false, description: 'Resume existing session' },
          max_steps: { type: 'number', required: false, min: 1, max: 4, description: 'Max planning steps (goal mode, default 4)' },
        },
      },
    },
  }
...
export function buildPaymentRequirements(resourceUrl: string, cfg: X402Config): PaymentRequirements {
  return {
    x402Version: 2,
    resource: { url: resourceUrl, description: 'ResearchKit multi-step research agent (v2.4.3). One-shot per call.' },
